...
export function getDifficulty(score = 0, elapsedSeconds = 0) {
  ...
  // Once the gap has smoothed almost all the way to its floor, breathe it a
  // little so a very long run doesn't settle into an identical corridor
  // forever. The outer clamp means this can only ever match or loosen the
  // floor, never tighten past it.
  const flutter = smooth >= 0.97 ? Math.sin(score * 0.4) * 6 : 0;
  const gap = clamp(o.gapStart - smooth * (o.gapStart - o.gapMin) + flutter, o.gapMin, o.gapMax);
  const spacing = clamp(o.initialSpacing - smooth * 42 + Math.sin(score * 0.31) * 16, o.minSpacing, o.initialSpacing);
  const variation = clamp(72 + score * 1.5, 72, 158);
  const movingChance = score < o.movingScore ? 0 : clamp((score - o.movingScore) / 160, 0.08, 0.4);
