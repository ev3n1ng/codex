...
const cachedFallbackMode = process.env.DATA_REFRESH_MODE === "cached-fallback";
...
function normalizeReportHtml(html, retailer, options = {}) {
  ...
  const dataStatus = options.cached
    ? "Cached fallback data - live retailer refresh did not update these prices"
    : "Live retailer data collected during this run";
...
function reportPage({
  ...
  dataStatus = "Live retailer data collected during this run",
}) {
...
const johnLewisReport = fs.existsSync("upload-index.html") && !cachedFallbackMode
...
const currysCachedMeta = cachedFallbackMode ? existingReportMeta("currys/index.html") : {};
